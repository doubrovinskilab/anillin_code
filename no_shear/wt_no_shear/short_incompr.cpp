	...
	for(i=0; ;i++)
	{
		...
		for(int ii=0; ii<1000; ii++)
		{
			...
			for(j=0; j<NumEdges; j++)
			{
				...
				//--------------------------------------------------------------------------------------------------------------------------------------------------
				// Elastic stress (on all, including basal membranes)
				...
				*/
				...
				else if( EdgeId[j]==3 ) // stress at basal membranes
				{
			...
			//------------------------------------------------------------------------------------------------------------------------------------------------------
			// Force of constraint to maintain total volume of the embryo (enclosed by the apical surface = cells + yolk)
			V = 0;
			...
			//------------------------------------------------------------------------------------------------------------------------------------------------------
			// Conserve volumes of individual cells
			for(j=0; j<NumCells; j++)
			{
				V = 0;
				for(k=0; k<NumPointsPerCell; k++)
				{
					int Idx = k;
					int Nxt = k+1;
					if( Nxt==NumPointsPerCell ) Nxt = 0;

					V += (BndryX[CellList[j][Nxt]]+BndryX[CellList[j][Idx]])*(BndryY[CellList[j][Nxt]]-BndryY[CellList[j][Idx]])/4 - (BndryY[CellList[j][Nxt]]+BndryY[CellList[j][Idx]])*(BndryX[CellList[j][Nxt]]-BndryX[CellList[j][Idx]])/4 ;
				}
				CellVs[j] = V;
			}
			...
			for(j=0; j<NumCells; j++)
			{
				...
				{
					int Prv = k-1;
						if( Prv<0 ) Prv = NumPointsPerCell-1;
					int Nxt = k+1;
						if( Nxt==NumPointsPerCell ) Nxt = 0;

					Bndry_fx[CellList[j][k]] += -(2.0e-4)*(BndryY[CellList[j][Nxt]]-BndryY[CellList[j][Prv]])*(CellVs[j]-CellV0);
					Bndry_fy[CellList[j][k]] += -(2.0e-4)*(BndryX[CellList[j][Prv]]-BndryX[CellList[j][Nxt]])*(CellVs[j]-CellV0);
				}
			}
			//------------------------------------------------------------------------------------------------------------------------------------------------------
