...
int NumCells = 100;
int NumPointsPerCell = 114;

double* CellVs = new double[NumCells];

int** CellList = new int*[NumCells];

for(i=0; i<NumCells; i++)
{
	CellList[i] = new int[NumPointsPerCell];
}

rd1 = new ifstream("./meshfiles/cell_list.dat");

for(i=0; i<NumCells; i++)
{
	for(j=0; j<NumPointsPerCell; j++)
	{
		rd1->getline(str1,100);

		CellList[i][j] = int( atof(str1) )-1;
	}
}
rd1->close();
delete rd1;
...
for(j=0; j<NumPointsPerCell; j++)
{
	...
	V += (BndryX[CellList[0][Nxt]]+BndryX[CellList[0][Idx]])*(BndryY[CellList[0][Nxt]]-BndryY[CellList[0][Idx]])/4 - (BndryY[CellList[0][Nxt]]+BndryY[CellList[0][Idx]])*(BndryX[CellList[0][Nxt]]-BndryX[CellList[0][Idx]])/4 ;
}
double CellV0 = V;